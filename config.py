...
class SystemInfoDialog(QDialog):
    def __init__(self, parent=None):
        ...
        graphics = subprocess.getoutput("if [[ $(nvidia-smi) ]]; then nvidia-smi -q | grep "Product Name" | sed 's/.*: //;s/\(.*\)/[\1]/'; else lspci | grep VGA | awk '{print $11, $12, $13}'; fi")
        temp_radeon = subprocess.getoutput("if [[ $(nvidia-smi) ]]; then nvidia-smi -q | grep "GPU Current Temp" | sed 's/.*: //;s/ C/ºC/'; else sensors | grep -A 0 'edge' | cut -c16-17; fi")
        temp_cpu = subprocess.getoutput("if [[ $(cat /proc/cpuinfo | grep "vendor_id" | head -n 1 | sed 's/.*: //') == 'GenuineIntel' ]]; then sensors | grep "Package" | sed 's/\..*//;s/.*+//'; else sensors | grep -A 0 'temp1' | cut -c16-17; fi")
